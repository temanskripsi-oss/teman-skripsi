...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: sales } = await supabase
      ...
      .select('id, code, sales_name, sales_password, is_active')
    ...
    const token = await new SignJWT({ id: sales.id, code: sales.code, name: sales.sales_name })
      .setProtectedHeader({ alg: 'HS256' })
      .setExpirationTime('7d')
      .sign(SECRET)

    const res = NextResponse.json({ ok: true, name: sales.sales_name, code: sales.code })
