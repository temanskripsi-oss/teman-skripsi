...
import { FlaskConical, CheckCircle, Loader2, CreditCard, Building2 } from 'lucide-react'
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-5">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between">
            ...
            <p className="text-lg font-bold text-[#0f766e]">Rp {PRICE.toLocaleString('id-ID')}</p>
          </div>
                ...
                <button key={m.code} type="button" onClick={() => setMethod(m.code)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all cursor-pointer ${
                    method === m.code
                      ...
                      : 'border-gray-200 hover:border-[#0f766e]/40 hover:bg-[#f0fdfa]'
                  }`}>
                  <div className={`p-1.5 rounded-lg flex-shrink-0 ${method === m.code ? 'bg-[#0f766e] text-white' : 'bg-gray-100 text-[#6B6B8A]'}`}>
                  ...
                  <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${method === m.code ? 'border-[#0f766e] bg-[#0f766e]' : 'border-gray-300'}`}>
